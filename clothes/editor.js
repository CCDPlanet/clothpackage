...
function getComponentsFromClothes(gender, layers) {
	...
	if (jacketItem) {
		...
		if (shirtItem && jacketItem.torso === 'torso_open') {
			...
			if ((shirtItem.neck === 'neck_open' || shirtItem.neck == 'neck_side'|| shirtItem.neck == 'neck_open_wide' || (!male && typeof undershirtId !== 'number')) && jacketItem.sleeve === 'sleeve_long') {
	...
	if (shirtItem && !jacketItem) {
		...
		if (!male && shirtItem.neck === 'neck_open_wide' && shirtItem.sleeve === 'sleeve_none') {
			components.set(3, [12, 0]);
		}
	}
